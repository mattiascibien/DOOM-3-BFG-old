...
void idMenuScreen_Shell_Dev::SetupDevOptions()
{
	...
	if (!f)
	{
		common->Printf("Cannot open %s\n", filename.c_str());
		devOptions.Append(devOption_t(NULL, "ERROR: no devlist"));
		return;
	}
	...
	parsed = new idCmdArgs(f, false);

	int i = 0;
	while (i < parsed->Argc())
	{
		devOptions.Append(devOption_t(strcmp(parsed->Argv(i), "NULL") == 0 ? NULL : parsed->Argv(i), parsed->Argv(i + 1)));
		i += 2;
	}
